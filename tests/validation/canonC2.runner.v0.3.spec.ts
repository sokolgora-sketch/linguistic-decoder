...
describe("Canon C2 v0.3 — anti-regression", () => {
  it("has no drift (or updates baseline in update mode)", async () => {
    ...
    const outCurrentPath = path.join(root, "tests/validation/out/canonC2.current.v0.3.json");
    const outDiffMdPath = path.join(root, "tests/validation/out/canonC2.diff.v0.3.md");
    ...
    fs.writeFileSync(outCurrentPath, stableJson({ version: "canonC2.current.v0.3", cases: currentById }) + "\n", "utf8");
