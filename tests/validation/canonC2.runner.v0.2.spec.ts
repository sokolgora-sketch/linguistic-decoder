...
async function callAnalyze(analyze: any, c: CanonCaseV01): Promise<any> {
...
describe("Canon C2 v0.2 — anti-regression", () => {
  it("has no drift (or updates baseline in update mode)", async () => {
    const update = process.env.CANON_C2_UPDATE === "1";

    const root = process.cwd();
    const trainPath = path.join(root, "tests/validation/datasets/canonC2.train.v0.2.json");
    const holdoutPath = path.join(root, "tests/validation/datasets/canonC2.holdout.v0.2.json");
    const baselinePath = path.join(root, "tests/validation/baselines/canonC2.baseline.v0.2.json");

    const outCurrentPath = path.join(root, "tests/validation/out/canonC2.current.v0.2.json");
    const outDiffMdPath = path.join(root, "tests/validation/out/canonC2.diff.v0.2.md");
    ...
    fs.writeFileSync(outCurrentPath, stableJson({ version: "canonC2.current.v0.2", cases: currentById }) + "\n", "utf8");
