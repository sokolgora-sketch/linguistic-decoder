...
> DeepSeek same-thread feedback adaptation battery.  
...
`tests/validation/out/evals.battery.2026-03-samethread/deepseek.deepthinking/`
...
|---|---:|---:|---:|---|
| r01 | 0.293574 | 0.035714 | 0.964917 | wrong-direction start |
| r02 | -0.527302 | -0.464286 | 0.304583 | best Pearson recovery |
| r03 | -0.331370 | -0.642857 | 0.143083 | best Spearman recovery |
| r04 | 0.231057 | 0.214286 | 0.655417 | wrong-direction relapse |
| r05 | -0.242268 | -0.250000 | 0.595833 | weak negative |
| r06 | -0.301335 | -0.321429 | 0.491333 | moderate negative |
| r07 | 0.288826 | 0.178571 | 0.718333 | wrong-direction relapse |
| r08 | -0.325192 | -0.285714 | 0.558417 | negative recovery |
| r09 | -0.083920 | -0.107143 | 0.831083 | near-neutral weak negative |
| r10 | 0.406721 | 0.464286 | 0.303417 | wrong-direction finish |
...
### Best available runs
1. `r02` — strongest Pearson
2. `r03` — strongest Spearman
3. `r06`
4. `r08`
...
### Important note
DeepSeek produced real negative recoveries, but they did not stabilize into a lasting strong cluster.

---

## Main interpretation

This DeepSeek same-thread batch is mixed and oscillatory.

What the 10 runs show:

- The sequence starts in a wrong-direction state at:
  - `r01`

- It then recovers strongly into negative territory at:
  - `r02`
  - `r03`

- After that, the sequence does not stabilize:
  - `r04` flips positive again
  - `r05` and `r06` recover negative
  - `r07` flips positive again
  - `r08` recovers negative
  - `r09` weakens toward near-neutral
  - `r10` flips positive again

So DeepSeek is clearly capable of producing usable negative runs under same-thread feedback, but it does **not** hold that state reliably.
...
That makes it better than Gemini under same-thread conditioning, but clearly less stable than a strong provider would need to be.
...
For DeepSeek, the striking observation is:
- the model can recover into the correct side
- but it does not reliably stay there
...
1. We kept one dedicated DeepSeek thread for `r01–r10`.
...
We intentionally stop this DeepSeek same-thread phase at **10 runs**.

Reason:
- 10 runs already expose:
  - wrong-direction starts
  - strong negative recoveries
  - repeated relapse into positive behavior
...
## Final DeepSeek same-thread conclusion

For `deepseek / deepthinking` on `T2_LADDER_V0_1` under same-thread feedback adaptation:

- Best observed runs: `r02`, `r03`
- Other usable negative runs: `r06`, `r08`, with weaker support from `r05` and `r09`
- Wrong-direction outputs remained recurrent: `r01`, `r04`, `r07`, `r10`
- Feedback produced real recoveries, but not stable convergence
- The model remained oscillatory under same-thread conditioning
...
Do **not** continue to `r11+` for DeepSeek same-thread right now.

Next move:
1. Finish saving / replacing all `r01–r10` PDFs
2. Copy DeepSeek same-thread artifacts from `out/` to `baselines/`
3. Commit / push / merge DeepSeek same-thread battery
4. Then run the remaining provider battery
