...
function runWithFixture(overrideFixturePath?: string): string {
  return execFileSync(process.execPath, [scriptPath], {
    ...
    env: {
      ...process.env,
      ...(overrideFixturePath
        ? { OI_FIRST_CONTROLLED_LOCAL_PROVIDER_EXECUTION_RUN_AUTHORIZATION_FIXTURE: overrideFixturePath }
        : {}),
    },
    encoding: "utf8",
  });
...
function writeMutatedFixture(mutator: (fixture: any) => void): string {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oi-first-run-auth-"));
...
describe("first controlled local-provider execution run authorization static validation", () => {
  it("passes the checked-in static fixture", () => {
    const output = runWithFixture();

    expect(output).toContain("First controlled local-provider execution run authorization validation passed");
    expect(output).toContain("first_controlled_local_provider_execution_run_authorization_contract_static");
    expect(output).toContain("defaultState=first_controlled_execution_run_authorization_not_granted");
    expect(output).toContain("oneShotRunGrantActive=false");
    ...
    expect(output).toContain("consumptionPolicyRequired=true");
    expect(output).toContain("expirationPolicyRequired=true");
  });
  ...
  it("fails closed on missing endpoint identity requirement", () => {
    expectFailure(
      (fixture) => {
        fixture.identityRequirements.endpointIdentityRequired = false;
      },
      "identityRequirements.endpointIdentityRequired must be true"
    );
  });
  ...
  it("fails closed on maximum request count drift", () => {
    expectFailure(
      (fixture) => {
        fixture.limits.maximumRequestCount = 2;
      },
      "limits.maximumRequestCount expected 1 got 2"
    );
  });

  it("fails closed on maximum response count drift", () => {
    expectFailure(
      (fixture) => {
        fixture.limits.maximumResponseCount = 2;
      },
      "limits.maximumResponseCount expected 1 got 2"
    );
  });
  ...
  it("fails closed on missing consumption policy", () => {
    expectFailure(
      (fixture) => {
        fixture.lifecyclePolicy.consumptionPolicyRequired = false;
      },
      "lifecyclePolicy.consumptionPolicyRequired must be true"
    );
  });

  it("fails closed on missing expiration policy", () => {
    expectFailure(
      (fixture) => {
        fixture.lifecyclePolicy.expirationPolicyRequired = false;
      },
      "lifecyclePolicy.expirationPolicyRequired must be true"
    );
  });

  it("fails closed on active one-shot run grant drift", () => {
    expectFailure(
      (fixture) => {
        fixture.states.active = "first_controlled_execution_run_authorization_granted_one_shot_local_only";
      },
  ...
  it("fails closed on post-run review requirement drift", () => {
    expectFailure(
      (fixture) => {
        fixture.postRunReview.requiredBeforeEvidenceClassChange = false;
      },
      "postRunReview.requiredBeforeEvidenceClassChange must be true"
    );
  });
});
