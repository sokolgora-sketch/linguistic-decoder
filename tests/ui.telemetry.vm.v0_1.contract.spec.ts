...
describe("ui telemetry vm contract v0.1", () => {
  ...
  it("keeps required top-level keys", () => {
    ...
    expect(Object.keys(vm).sort()).toEqual(
      ["analysisStatusV0_1", "candidates", "doctrineReading", "evidence", "math", "originClaim", "originClaimGates", "raw", "readout", "rejections", "resonanceProfileV1", "rootMap", "soundRoots"].sort()
    );
  });
});
