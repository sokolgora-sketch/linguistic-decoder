...
describe("Semantic Pilot Albanian200 MATRIX v0.1 — Tag×Vowel p-matrix (Albanian (sq), IPA required)", () => {
  it("writes tests/validation/out/semanticPilot.albanian200.matrix.v0.1.md", () => {
    const root = process.cwd();
    const wordsPath = path.join(root, "tests/research/albanian200.words.v0.1.txt");
    ...
    const outMd = path.join(outDir, "semanticPilot.albanian200.matrix.v0.1.md");
    ...
    const lines: string[] = [];
    lines.push("# Semantic Pilot Albanian200 MATRIX v0.1 — Tag×Vowel p-matrix (Albanian (sq))");
    lines.push("");
    lines.push(`- corpus: Albanian200 (200)`);
