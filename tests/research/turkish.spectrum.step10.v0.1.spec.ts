...
describe("Turkish Spectrum — STEP10 v0.1 (orthography slope probe; langHint=tr)", () => {
  it("writes STEP10 out reports; writes baselines only when STEP10 is complete", () => {
    const root = process.cwd();
    const inPath = path.join(root, "tests/research/turkish.spectrum.step10.v0.1.txt");

    const outDir = path.join(root, "tests/validation/out");
    const outMd = path.join(outDir, "turkish.spectrum.step10.v0.1.md");
    const outJson = path.join(outDir, "turkish.spectrum.step10.v0.1.json");

    const baseDir = path.join(root, "tests/validation/baselines");
    const baseMd = path.join(baseDir, "turkish.spectrum.step10.v0.1.md");
    const baseJson = path.join(baseDir, "turkish.spectrum.step10.v0.1.json");
    ...
    const md: string[] = [];
    md.push(`# Turkish Spectrum — STEP10 v0.1 (langHint=tr)`);
    md.push("");
    md.push(`- corpus: \`tests/research/turkish.spectrum.step10.v0.1.txt\``);
    ...
    md.push("## Bucket counts (TARGET=10 per bucket)");
    md.push("");
    ...
    md.push("|--------|-------|----------:|---------:|-----------:|---------:|");
    md.push(`| ${slopeAll.label} | aperture(primary) | ${fmt(slopeAll.sP.obsR, 3)} | ${fmt(slopeAll.sP.pR, 3)} | ${fmt(slopeAll.sP.obsRs, 3)} | ${fmt(slopeAll.sP.pRs, 3)} |`);
    md.push(`| ${slopeAll.label} | aperture(presence mean) | ${fmt(slopeAll.sM.obsR, 3)} | ${fmt(slopeAll.sM.pR, 3)} | ${fmt(slopeAll.sM.obsRs, 3)} | ${fmt(slopeAll.sM.pRs, 3)} |`);
    md.push("");
    ...
    // JSON report
    const j = {
      version: "turkish.spectrum.step10.v0.1",
