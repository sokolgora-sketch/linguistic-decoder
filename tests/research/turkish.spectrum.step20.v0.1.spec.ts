...
describe("Turkish Spectrum — STEP20 v0.1 (orthography slope probe; langHint=tr)", () => {
  it("writes STEP20 out reports; writes baselines only when STEP20 is complete; writes compare when complete", () => {
    const root = process.cwd();
    const inPath = path.join(root, "tests/research/turkish.spectrum.step20.v0.1.txt");

    const outDir = path.join(root, "tests/validation/out");
    const outMd = path.join(outDir, "turkish.spectrum.step20.v0.1.md");
    const outJson = path.join(outDir, "turkish.spectrum.step20.v0.1.json");

    const baseDir = path.join(root, "tests/validation/baselines");
    const baseMd = path.join(baseDir, "turkish.spectrum.step20.v0.1.md");
    const baseJson = path.join(baseDir, "turkish.spectrum.step20.v0.1.json");

    const cmpOutMd = path.join(outDir, "turkish.spectrum.step20.v0.1.compare.v0.1.md");
    const cmpOutJson = path.join(outDir, "turkish.spectrum.step20.v0.1.compare.v0.1.json");
    const cmpBaseMd = path.join(baseDir, "turkish.spectrum.step20.v0.1.compare.v0.1.md");
    const cmpBaseJson = path.join(baseDir, "turkish.spectrum.step20.v0.1.compare.v0.1.json");

    const step10BaseJson = path.join(baseDir, "turkish.spectrum.step10.v0.1.json");
    ...
    const items: Item[] = rows.map((r) => {
      const raw = extractOrthographyVoicesFromWordV0_1({ word: r.word, langHint: "tr" }).voices;
      const vs0 = (Array.isArray(raw) ? raw : []).map(normVowel).filter(Boolean) as Vowel[];
      const vs = uniqInOrder(vs0);

      if (!vs.length) throw new Error(`NO_VOWELS (orthography; tr) id=${r.id} word=${r.word}`);
    ...
    const md: string[] = [];
    md.push(`# Turkish Spectrum — STEP20 v0.1 (langHint=tr)`);
    md.push("");
    md.push(`- corpus: \`tests/research/turkish.spectrum.step20.v0.1.txt\``);
    ...
    md.push("|--------|-------|----------:|---------:|-----------:|---------:|");
    md.push(
      `| ${slopeAll.label} | aperture(primary) | ${fmt(slopeAll.sP.obsR, 3)} | ${fmt(slopeAll.sP.pR, 3)} | ${fmt(slopeAll.sP.obsRs, 3)} | ${fmt(slopeAll.sP.pRs, 3)} |`
    );
    md.push(
      `| ${slopeAll.label} | aperture(presence mean) | ${fmt(slopeAll.sM.obsR, 3)} | ${fmt(slopeAll.sM.pR, 3)} | ${fmt(slopeAll.sM.obsRs, 3)} | ${fmt(slopeAll.sM.pRs, 3)} |`
    );
    md.push("");
    ...
    // JSON report
    const j = {
      version: "turkish.spectrum.step20.v0.1",
    ...
    // Compare (STEP10 baseline -> STEP20 current) only when STEP20 complete
    const cmpMd: string[] = [];
    const cmp = {
      version: "turkish.spectrum.step20.v0.1.compare.v0.1",
      step10: { baselineJson: "tests/validation/baselines/turkish.spectrum.step10.v0.1.json" },
      step20: { corpus: "tests/research/turkish.spectrum.step20.v0.1.txt", baselineReady },
      delta: {},
    } as any;

    cmpMd.push("# Turkish Spectrum — Compare STEP10→STEP20 (v0.1)");
    cmpMd.push("");
    cmpMd.push(`- step10 baseline: \`tests/validation/baselines/turkish.spectrum.step10.v0.1.json\``);
    cmpMd.push(`- step20 corpus: \`tests/research/turkish.spectrum.step20.v0.1.txt\``);
    cmpMd.push(`- compare write: ${baselineReady ? "ENABLED (STEP20 complete)" : "DISABLED (STEP20 incomplete)"}`);
    cmpMd.push("");

    if (!fs.existsSync(step10BaseJson)) {
      cmpMd.push("Step10 baseline JSON missing; compare cannot run.");
    ...
    if (!baselineReady) {
      cmpMd.push("STEP20 is not complete yet (TARGET=20 per bucket). Compare is deferred.");
      fs.writeFileSync(cmpOutMd, cmpMd.join("\n"), "utf8");
      fs.writeFileSync(cmpOutJson, JSON.stringify(cmp, null, 2) + "\n", "utf8");
      return;
    }

    const step10 = JSON.parse(fs.readFileSync(step10BaseJson, "utf8"));
    const s10P = step10?.cohorts?.all?.slope?.sP;
    const s10M = step10?.cohorts?.all?.slope?.sM;

    const s20P = slopeAll.sP;
    const s20M = slopeAll.sM;

    function pick(x: any) {
      return x
        ? { r: x.obsR, p: x.pR, rho: x.obsRs, pRho: x.pRs }
        : { r: NaN, p: NaN, rho: NaN, pRho: NaN };
    }
    ...
    function abs(x: number) {
      return Number.isFinite(x) ? Math.abs(x) : NaN;
    }

    cmp.delta = {
      primary: { absR10: abs(p10.r), absR20: abs(p20.r), dAbsR: abs(p20.r) - abs(p10.r), absRho10: abs(p10.rho), absRho20: abs(p20.rho), dAbsRho: abs(p20.rho) - abs(p10.rho) },
      presenceMean: { absR10: abs(m10.r), absR20: abs(m20.r), dAbsR: abs(m20.r) - abs(m10.r), absRho10: abs(m10.rho), absRho20: abs(m20.rho), dAbsRho: abs(m20.rho) - abs(m10.rho) },
    };

    cmpMd.push("## Slope comparison (ALL)");
    cmpMd.push("");
    cmpMd.push("| Score | STEP10 r | STEP10 p | STEP20 r | STEP20 p | STEP10 ρ | STEP10 p | STEP20 ρ | STEP20 p |");
    cmpMd.push("|------|----------:|---------:|----------:|---------:|---------:|---------:|---------:|---------:|");
    cmpMd.push(`| aperture(primary) | ${fmt(p10.r)} | ${fmt(p10.p)} | ${fmt(p20.r)} | ${fmt(p20.p)} | ${fmt(p10.rho)} | ${fmt(p10.pRho)} | ${fmt(p20.rho)} | ${fmt(p20.pRho)} |`);
    cmpMd.push(`| aperture(presence mean) | ${fmt(m10.r)} | ${fmt(m10.p)} | ${fmt(m20.r)} | ${fmt(m20.p)} | ${fmt(m10.rho)} | ${fmt(m10.pRho)} | ${fmt(m20.rho)} | ${fmt(m20.pRho)} |`);
    cmpMd.push("");
    cmpMd.push("## Delta on absolute strength (higher is stronger)");
    cmpMd.push("");
    cmpMd.push("| Score | |r| STEP10 | |r| STEP20 | Δ|r| | |ρ| STEP10 | |ρ| STEP20 | Δ|ρ| |");
    cmpMd.push("|------|----------:|----------:|----:|----------:|----------:|----:|");
    cmpMd.push(`| aperture(primary) | ${fmt(cmp.delta.primary.absR10)} | ${fmt(cmp.delta.primary.absR20)} | ${fmt(cmp.delta.primary.dAbsR)} | ${fmt(cmp.delta.primary.absRho10)} | ${fmt(cmp.delta.primary.absRho20)} | ${fmt(cmp.delta.primary.dAbsRho)} |`);
    cmpMd.push(`| aperture(presence mean) | ${fmt(cmp.delta.presenceMean.absR10)} | ${fmt(cmp.delta.presenceMean.absR20)} | ${fmt(cmp.delta.presenceMean.dAbsR)} | ${fmt(cmp.delta.presenceMean.absRho10)} | ${fmt(cmp.delta.presenceMean.absRho20)} | ${fmt(cmp.delta.presenceMean.dAbsRho)} |`);
    cmpMd.push("");

    fs.writeFileSync(cmpOutMd, cmpMd.join("\n"), "utf8");
    fs.writeFileSync(cmpOutJson, JSON.stringify(cmp, null, 2) + "\n", "utf8");

    // lock compare baselines only when STEP20 is complete
