...
describe("Pseudowords Spectrum — STEP10 v0.1 (orthography slope probe; negative control)", () => {
  it("writes STEP10 out reports; writes baselines only when STEP10 is complete", () => {
    const root = process.cwd();
    const inPath = path.join(root, "tests/research/pseudowords.spectrum.step10.v0.1.txt");

    const outDir = path.join(root, "tests/validation/out");
    const outMd = path.join(outDir, "pseudowords.spectrum.step10.v0.1.md");
    const outJson = path.join(outDir, "pseudowords.spectrum.step10.v0.1.json");

    const baseDir = path.join(root, "tests/validation/baselines");
    const baseMd = path.join(baseDir, "pseudowords.spectrum.step10.v0.1.md");
    const baseJson = path.join(baseDir, "pseudowords.spectrum.step10.v0.1.json");
    ...
    const md: string[] = [];
    md.push(`# Pseudowords Spectrum — STEP10 v0.1`);
    md.push("");
    md.push(`- purpose: negative control (no semantic intent; bucket assignment independent of aperture)`);
    md.push(`- corpus: \`tests/research/pseudowords.spectrum.step10.v0.1.txt\``);
    ...
    // JSON report
    const j = {
      version: "pseudowords.spectrum.step10.v0.1",
