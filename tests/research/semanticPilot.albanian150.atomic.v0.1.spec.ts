...
describe("Semantic Pilot Albanian150 ATOMIC v0.1 — tag ↔ vowel association (Albanian (sq), IPA required)", () => {
  it("writes tests/validation/out/semanticPilot.albanian150.atomic.v0.1.md", () => {
    const root = process.cwd();

    const wordsPath = path.join(root, "tests/research/albanian150.words.v0.1.txt");
    const metaPath = path.join(root, "tests/research/albanian150.meta.v0.2.atomic.json");
    const outDir = path.join(root, "tests/validation/out");
    const outMd = path.join(outDir, "semanticPilot.albanian150.atomic.v0.1.md");
...
const meta = readJson<Meta>(metaPath);
    ...
    const lines: string[] = [];
    lines.push("# Semantic Pilot Albanian150 ATOMIC v0.1 — Tag ↔ Vowel Purity (Albanian (sq))");
    lines.push("");
