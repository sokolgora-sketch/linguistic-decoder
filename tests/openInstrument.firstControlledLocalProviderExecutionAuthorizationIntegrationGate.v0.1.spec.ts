...
describe("first controlled local-provider execution authorization integration gate", () => {
  ...
  it("keeps fixture non-executing and one-shot grant inactive", () => {
    const fixture = JSON.parse(fs.readFileSync(fixturePath, "utf8"));

    expect(fixture.status).toBe("static_contract_only");
    expect(fixture.states.default).toBe("first_controlled_execution_authorization_not_granted");
    expect(fixture.states.active).toBe("first_controlled_execution_authorization_not_granted");
    expect(fixture.states.oneShotGrantActive).toBe(false);
  ...
  it("keeps schema anchored on static non-executing contract markers", () => {
    const schema = JSON.parse(fs.readFileSync(schemaPath, "utf8"));

    expect(schema.title).toContain("first controlled local-provider execution authorization");
    ...
    expect(schema.properties.states.properties.oneShotGrantActive.const).toBe(false);
