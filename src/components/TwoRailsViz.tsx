...
const TwoRailsViz = forwardRef<TwoRailsVizHandle, TwoRailsVizProps>(({ initialWord, initialVoicePath }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    triggerAnalysis(res: EngineResult) {
      if (p5InstanceRef.current && p5InstanceRef.current.injectEngineResult) {
        p5InstanceRef.current.injectEngineResult(res);
      }
    },
  }));
  ...
  useEffect(() => {
    ...
    const initSketch = async () => {
        ...
        p.injectEngineResult = (res: EngineResult) => {
        ...
        function buildConsonantBuckets(word: string, hops: number) {
            ...
            for (let i = 0; i < seq.length; i++) {
                if (buckets[i % hops]) {
                    (buckets[i % hops] as any[]).push(seq[i]);
                }
            }
            return buckets;
        }
  ...
  }, [initialWord, initialVoicePath]); // Run only once
