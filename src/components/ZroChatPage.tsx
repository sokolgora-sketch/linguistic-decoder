...
function VoiceRow() {
  // Intentionally NOT A/E/I/O/U/Y/Ë order.
  // More "ring-ish": center → inner → middle → outer.
...
export default function ZroChatPage() {
  const [input, setInput] = React.useState("study");
  ...
  async function run(wordRaw: string) {
    ...
    try {
        ...
        setMessages((m) =>
          m.map((x) =>
            x.id === assistantMsg.id
              ? { ...x, text: "Error.", error: String(err), result: json ?? undefined }
              : x
          )
        );
        return;
      ...
      setMessages((m) =>
        m.map((x) =>
          x.id === assistantMsg.id
            ? { ...x, text: "Result:", result: json ?? undefined }
            : x
        )
      );
    } catch (e: any) {
      setMessages((m) =>
        m.map((x) =>
          x.id === assistantMsg.id
            ? { ...x, text: "Error.", error: e?.message || "Request failed." }
            : x
        )
      );
    } finally {
  ...
  const composer = (
    <div className="space-y-2">
      {/* Keep this label text for continuity with your existing UI/tests */}
      ...
      <div className="flex gap-2">
        <Input
          ...
          className="h-11"
        />
        <Button className="h-11" onClick={() => run(input)} disabled={busy}>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {messages.map((m) => {
            ...
            const wordShown =
              result?.word ?? result?.normalizedWord ?? "";
                ...
                <div className="w-full max-w-3xl">
                  <Card className={isUser ? "border-muted" : ""}>
                    <CardContent className="py-4 space-y-3">
                      ...
                      <div className={isUser ? "text-base font-medium" : "text-sm"}>
                        {m.text}
                      </div>
