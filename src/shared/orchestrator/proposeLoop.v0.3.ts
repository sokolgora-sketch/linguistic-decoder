...
export async function proposeLoopV0_3(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const systemPrompt = buildRootProposerSystemPromptV0_2({ failReasons: lastFailReasons });

    const userPayload = lastFailReasons.length
      ? { word, mode, repair: { failReasons: lastFailReasons } }
      : { word, mode };
