...
describe("buildRootMapV1", () => {
  ...
  it("should build a RootMap from the first minRoots hypothesis (dama)", () => {
    const minRoots: MinRootHypothesis[] = [
      {
        ...
        protoRoots: ["DA", "MA"],
        carriers: [
          { protoRootId: "DA", segment: "da", lang: "Albanian", carrierForm: "da", ops: [] },
          { protoRootId: "MA", segment: "ma", lang: "Albanian", carrierForm: "ma", ops: [] },
        ],
    ...
    expect(result?.composedMeaning).toBe("split / divide + measure");
    expect(result?.tokens.map((t) => t.token)).toEqual(["DA", "MA"]);
    expect(result?.keys[0].language).toBe("Albanian");
