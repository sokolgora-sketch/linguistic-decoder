...
function renderIntermediate(
  ...
  return [
    ...
    `- verdict: ${s.verdict}`,
    `- order permutation p: ${fmtP(s.ordinalPermutation.p_value)}`,
    `- order permutation test: iters=${s.ordinalPermutation.iters}, seed=${s.ordinalPermutation.seed}, observedOrder=${s.ordinalPermutation.observed_order}`,
    `- margin permutation p: ${fmtP(s.marginPermutation.p_value)}`,
    `- observed min gap: ${fmt(s.marginPermutation.observed_min_gap)}`,
    `- margin permutation test: iters=${s.marginPermutation.iters}, seed=${s.marginPermutation.seed}`,
    `- Hedges g (low vs x): ${s.effectSizes.hedges_g_low_x === null ? "—" : fmt(s.effectSizes.hedges_g_low_x)}`,
    `- Hedges g (x vs high): ${s.effectSizes.hedges_g_x_high === null ? "—" : fmt(s.effectSizes.hedges_g_x_high)}`,
    `- bootstrap CI gap_low: ${s.bootstrap.ci95_gap_low ? `[${fmt(s.bootstrap.ci95_gap_low[0])}, ${fmt(s.bootstrap.ci95_gap_low[1])}]` : "—"}`,
