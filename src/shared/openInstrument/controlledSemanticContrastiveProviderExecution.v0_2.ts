...
export const CONTROLLED_SEMANTIC_CONTRASTIVE_EXECUTION_VERSION_V0_2 =
  "open-instrument.controlled-semantic-contrastive-execution.v0_2" as const;

type HistoricalPacketV0_1 = import("@/shared/openInstrument/controlledSemanticContrastiveProviderExecution.v0_1").ControlledSemanticContrastiveExecutionPacketV0_1;
type HistoricalAuthorizationV0_1 = import("@/shared/openInstrument/controlledSemanticContrastiveProviderExecution.v0_1").ControlledSemanticContrastiveAuthorizationV0_1;
type HistoricalRowV0_1 = import("@/shared/openInstrument/controlledSemanticContrastiveProviderExecution.v0_1").ControlledSemanticContrastiveExecutionRowV0_1;

export type ControlledSemanticContrastiveExecutionPacketV0_2 = Omit<HistoricalPacketV0_1, "controlledExecutionVersion"> & {
  controlledExecutionVersion: typeof CONTROLLED_SEMANTIC_CONTRASTIVE_EXECUTION_VERSION_V0_2;
};

export type ControlledSemanticContrastiveAuthorizationV0_2 = Omit<HistoricalAuthorizationV0_1, "controlledExecutionVersion"> & {
  controlledExecutionVersion: typeof CONTROLLED_SEMANTIC_CONTRASTIVE_EXECUTION_VERSION_V0_2;
};

export type ControlledSemanticContrastiveExecutionRowV0_2 = HistoricalRowV0_1;

export type ControlledSemanticContrastiveExecutionResultV0_2 = Readonly<{
  controlledExecutionVersion: typeof CONTROLLED_SEMANTIC_CONTRASTIVE_EXECUTION_VERSION_V0_2;
  ...
  rows: readonly ControlledSemanticContrastiveExecutionRowV0_2[];
}>;

type ContrastiveExecutorV0_2 = (
  context: ContrastiveSemanticContextV0_1,
  timeoutMs: number,
) => Promise<ContrastiveSemanticProposalResultV0_1>;

type RuntimePreflightExecutorV0_2 = (
  providerId: string,
  modelId: string,
  endpointUrl: string,
  localOnly: boolean,
...
type RunOptionsV0_2 = Readonly<{
  execute?: ContrastiveExecutorV0_2;
  runtimePreflight?: RuntimePreflightExecutorV0_2;
}>;

const consumedAuthorizationIdsV0_2 = new Set<string>();

function textV0_2(value: unknown): string {
  return typeof value === "string" ? value.normalize("NFKC").trim() : "";
}

function isLoopbackUrlV0_2(value: string): boolean {
  try {
...
export function fingerprintControlledSemanticContrastiveExecutionPacketV0_2(
  packet: ControlledSemanticContrastiveExecutionPacketV0_2,
): string {
...
export function validateControlledSemanticContrastiveExecutionPacketV0_2(
  packet: ControlledSemanticContrastiveExecutionPacketV0_2,
): readonly string[] {
  const reasons: string[] = [];
  const words = new Set(packet.pairs.map((pair) => textV0_2(pair.word)));
  ...
  if (packet.controlledExecutionVersion !== CONTROLLED_SEMANTIC_CONTRASTIVE_EXECUTION_VERSION_V0_2) reasons.push("CONTRASTIVE_EXECUTION_VERSION_INVALID");
  ...
  if (!packet.localOnly || !isLoopbackUrlV0_2(packet.endpointUrl)) reasons.push("CONTRASTIVE_LOCAL_ONLY_ENDPOINT_REQUIRED");
  ...
  if (packet.pairs.some((pair) => !textV0_2(pair.word) || !textV0_2(pair.structuralHypothesisId) || !textV0_2(pair.senseA.targetSenseId) || !textV0_2(pair.senseB.targetSenseId) || !textV0_2(pair.senseA.targetSenseLabel) || !textV0_2(pair.senseB.targetSenseLabel))) reasons.push("CONTRASTIVE_PAIR_FIELDS_REQUIRED");
...
function blockedV0_2(
  reasonCodes: readonly string[],
  plannedCalls: number,
  runtimePreflight: LocalProviderRuntimePreflightV0_1 | null = null,
): ControlledSemanticContrastiveExecutionResultV0_2 {
  return {
...
export async function runControlledSemanticContrastiveProviderExecutionV0_2(
  packet: ControlledSemanticContrastiveExecutionPacketV0_2,
  authorization: ControlledSemanticContrastiveAuthorizationV0_2,
  options: RunOptionsV0_2 = {},
): Promise<ControlledSemanticContrastiveExecutionResultV0_2> {
  const packetReasons = validateControlledSemanticContrastiveExecutionPacketV0_2(packet);
  const fingerprint = fingerprintControlledSemanticContrastiveExecutionPacketV0_2(packet);
  const authReasons = [
    ...
    consumedAuthorizationIdsV0_2.has(authorization.authorizationId) ? "CONTRASTIVE_AUTHORIZATION_CONSUMED" : "",
  ...
  if (packetReasons.length || authReasons.length) return blockedV0_2([...packetReasons, ...authReasons], packet.pairs.length);

  if (!options.execute || process.env.NODE_ENV !== "test") {
    const endpoint = process.env.OPENAI_BASE_URL?.trim() ?? "";
    if (endpoint !== packet.endpointUrl) return blockedV0_2(["CONTRASTIVE_PROVIDER_ENDPOINT_MISMATCH"], packet.pairs.length);
  }
  ...
  for (const pair of packet.pairs) {
    ...
    if (!structural) return blockedV0_2(["CONTRASTIVE_FROZEN_STRUCTURAL_INPUT_INVALID"], packet.pairs.length, runtimePreflight);
    const contextResult = buildContrastiveSemanticContextV0_1(pair, structural);
    if (!contextResult.ok) return blockedV0_2(["CONTRASTIVE_CONTEXT_INVALID", ...contextResult.reasonCodes], packet.pairs.length, runtimePreflight);
    contexts.push({ pair, context: contextResult.context });
  }

  consumedAuthorizationIdsV0_2.add(authorization.authorizationId);
  ...
  const rows: ControlledSemanticContrastiveExecutionRowV0_2[] = [];
