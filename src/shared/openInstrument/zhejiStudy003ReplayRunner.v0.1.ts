...
function buildSystemPrompt(): string {
  return [
    ...
    "For non-null chunkCandidates, language, candidateForm, meaning, and sourceNote must be non-empty.",
    "If any required non-null candidate payload field cannot be filled honestly, use nullCandidates with candidateType null_candidate instead.",
    ...
    "Every object in nullCandidates must use candidateType exactly null_candidate.",
    "Do not use opaque, unknown, none, empty string, weak_resonance, or phonetic_only as nullCandidates[].candidateType.",
    "null_candidate means no candidate was found for that chunk; opaque means a non-null candidate exists but cannot clearly motivate function.",
    "If structural fields are missing, the result is STRUCTURAL_FAILURE.",
    "</STRUCTURAL_CONTRACT>",
    "",
    "<LINGUISTIC_EVALUATION_RULES>",
    "Open Instrument is not an etymology task. Do not find origin. Do not choose a winner.",
    "Find candidates that can motivate the meaning/function of embryo morphemes through each language\'s own smallest meaningful units.",
    ...
    "Use Code F for the formal layer and Code E for the symbolic layer terminology.",
    ...
    "If semanticTransparency.level is uncertain for a non-null candidate, use opaque.",
    "Do not leave semanticTransparency.level empty or null.",
...
function buildUserPrompt(
  ...
  return [
    ...
    "Every chunkCandidates[] object must include segmentationId.",
    `Every chunkCandidates[].segmentationId must exactly equal ${ZHEJI_STUDY003_HEART_INPUT_V0_1.segmentationId}.`,
    "Every nullCandidates[] object must include segmentationId.",
    `Every nullCandidates[].segmentationId must exactly equal ${ZHEJI_STUDY003_HEART_INPUT_V0_1.segmentationId}.`,
    "Do not leave candidate-level segmentationId empty.",
    "Do not infer, shorten, translate, normalize, or invent segmentationId.",
    "Copy segmentationId exactly from Heart-approved input into every candidate and null-candidate object.",
    "nullCandidates[].candidateType must be null_candidate, not opaque.",
    "For non-null candidates, semanticTransparency.level must be atomic, metaphorical, or opaque; if uncertain, use opaque.",
