...
function loadDataset(): ValidationRecordV01[] {
  const p = path.join(process.cwd(), "tests/validation/datasets/validation.dataset.v0.2.json");
...
function main() {
  ...
  const fullRes = computeValidationResultsV0_2(all);
  const trainRes = computeValidationResultsV0_2(train);
  const holdoutRes = computeValidationResultsV0_2(holdout);

  const current = {
    version: "0.2",
    datasetId: "validation.dataset.v0.2",
  ...
  const outJson = "docs/validation/validation.results.current.v0.2.json";
  writeJson(outJson, current);

  const baselinePath = "docs/validation/validation.results.baseline.v0.2.json";
  ...
  const r = fullRes;
  const lines: string[] = [];
  lines.push(`# ZË-RO External Validation Report v0.2`);
  lines.push("");
  ...
  writeText("docs/validation/VALIDATION_REPORT_LATEST_v0.2.md", lines.join("\n"));
}
