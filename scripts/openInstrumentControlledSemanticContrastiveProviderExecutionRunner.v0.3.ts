...
} from "@/shared/openInstrument/controlledSemanticContrastiveProviderExecution.v0_3";

const repoRoot = process.cwd();
const packetPath = process.env.OI_CONTROLLED_CONTRASTIVE_SEMANTIC_PACKET ?? path.join(repoRoot, "docs/open-instrument/fixtures/controlled-semantic-contrastive-run/open-instrument-controlled-semantic-contrastive-run-packet-v0.3.json");
...
async function main(): Promise<void> {
  ...
  const packet = readJson<ControlledSemanticContrastiveExecutionPacketV0_3>(packetPath);
  const authorization = readJson<ControlledSemanticContrastiveAuthorizationV0_3>(authorizationPath);
